...
sys.path.append('/work/jdy/davin/proj/Tests/Models/blackbox_model_training/image')
from model import * 
from train import train_network
from nn_datasets import * 
sys.path.append('/work/jdy/davin/proj/')
...
def label_trainset_with_model(args, model, train_loader):
    ...
    model.eval() 
    print("Labeling Dataset With Model")
    model_labels = []
    data_list = []
    class_count = []
    min_samples = max(args.num_attack_untarg, args.num_attack_targ) # minimum number of samples per class
    num_samples_error = 0.95 # percentage of classes that must contain have min_samples. This is in case of very sparse classes.
    ...
    with torch.no_grad(): 
        for i, (data,labels) in enumerate(Train_Bar):
            ...
            # # Filter samples for predictions in target_class_list
            # import pdb; pdb.set_trace()
            # idx_list = []
            # for i, pred in enumerate(preds.reshape(-1,1)):
            #     if pred in targ_class_list: idx_list.append(i)
            # import pdb; pdb.set_trace()
            # preds = preds[np.array(idx_list)]

            # end early if the number of samples in each predicted class exceeds num_attack_targ
            if i == 0:
                class_count = torch.bincount(preds, minlength=len(class_count))
            else:
                tmp = torch.bincount(preds, minlength=len(class_count))
                if len(class_count) < len(tmp):
                    diff = len(tmp) - len(class_count)
                    class_count = torch.cat((class_count, torch.zeros((diff))))
            ...
            if (torch.quantile(class_count.float(), q = 1-num_samples_error) >= min_samples) and (class_count.min() > 0):
                break
    model_labels = torch.concat(model_labels, dim = 0)
    data_list = torch.concat(data_list, dim = 0)
    return data_list.float(), model_labels.type(torch.int64)
...
def targeted_attack(args, model, inputs, target_class):
    ...
    for data, target_class_batch in Adv_Bar:
        ...
        if args.eps_use == 'first':
            ...
            clipped_advs = torch.flatten(clipped_advs, end_dim = 1) # combine first two dimensions for efficiency
...
def generate_adverserial_for_class_wrapper(args, model, train_loader):

    
    # args = vars(args) # convert to dict
    num_targ_classes = vars(args).get('num_targ_classes', 0) # reduce the number of classes to sample for targeted attacks
    class_list = vars(args).get('class_list', None) # only calculate db for specified classes
    # targ_class_list = np.unique(utils_torch.tensor2numpy(tr_data_lab)).tolist()
    tr_data_im, tr_model_labels = label_trainset_with_model(args, model, train_loader)
    ...
    if class_list is None: # if not using a specified subset of classes
        class_list = classes
                
    tr_data_im = tr_data_im.float()
    ...
    for c in class_list:
        ...
        for c_f in tqdm(np.random.permutation(classes)[:num_targ_classes], total = num_targ_classes): 
            if not c == c_f:
